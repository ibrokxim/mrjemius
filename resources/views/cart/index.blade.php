...
@push('styles')
    ...
    <style>
        ...
        .checkout-button-final { background-color: #0aad0a; border-color: #0aad0a; font-size: 1rem; font-weight: 500; }
        .checkout-button-final:hover { background-color: #008a00; border-color: #008a00; }
    ...
    <main>
        ...
        <section class="mb-lg-14 mb-8 mt-8">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="card py-1 border-0 mb-8">
                            ...
                            <p class="mb-0">В вашей корзине <span id="cart-item-count-text">{{ $cartItems->sum('quantity') }}</span> товар(ов).</p>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="py-3">
                                ...
                                <div id="free-shipping-alert">
                                    @if($subtotal >= $freeShippingThreshold)
                                        <div class="alert alert-success p-2">У вас БЕСПЛАТНАЯ доставка!</div>
                                    @elseif($subtotal > 0)
                                        <div class="alert alert-info p-2">До бесплатной доставки осталось: <b>{{ number_format($needsForFreeShipping, 0, '.', ' ') }} сум</b>.</div>
                                    @endif
                                </div>
                                            ...
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="text-end ms-3" style="min-width: 90px;">
                                                        <span class="fw-bold text-dark item-total-price" data-item-id="{{ $item->id }}">{{ number_format(($item->product->sell_price ?? $item->product->price) * $item->quantity, 0, '.', ' ') }} сум</span>
                                                    </div>
                                    ...
                                    <div class="card mb-2">
                                        <ul class="list-group list-group-flush">
                                            ...
                                            <li class="list-group-item d-flex flex-column py-3">
                                                ...
                                                <div class="d-flex justify-content-between w-100">
                                                    ...
                                                    <span id="summary-shipping">
                                                        @if($shippingCost > 0)
                                                            {{ number_format($shippingCost, 0, '.', ' ') }} сум
                                                        @else
                                            ...
                                            <li class="list-group-item d-flex justify-content-between fw-bold fs-5">
                                                <span>Итого:</span>
                                                <span id="summary-total">{{ number_format($total, 0, '.', ' ') }} сум</span>
                                            </li>
                                        </ul>
                                    </div>
                ...
                @else
                    <div class="row"><div class="col-12 text-center py-8"><h2>Ваша корзина пуста</h2><a href="{{ route('welcome') }}" class="btn btn-primary mt-2">Начать покупки</a></div></div>
                @endif
            </div>
        </section>
    </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateCartUI(summary) {
                ...
                if (countTextEl) {
                    countTextEl.textContent = `${summary.count} товар(ов).`;
                }

                // Обновляем блок с итогами
                document.getElementById('summary-subtotal').textContent = `${formatNumber(summary.subtotal)} сум`;
                document.getElementById('summary-total').textContent = `${formatNumber(summary.total)} сум`;
                document.getElementById('checkout-total-btn').textContent = `${formatNumber(summary.total)} сум`;

                // Обновляем стоимость доставки с логикой
                const shippingEl = document.getElementById('summary-shipping');
                if (summary.shipping > 0) {
                    shippingEl.innerHTML = `${formatNumber(summary.shipping)} сум`;
                ...
                if (summary.subtotal >= freeShippingThreshold) {
                    ...
                    alertEl.innerHTML = `<div class="alert alert-info p-2">До бесплатной доставки осталось: <b>${formatNumber(summary.needsForFreeShipping)} сум</b>.</div>`;
                } else {
            ...
            async function sendCartUpdateRequest(itemId, quantity) {
                ...
                try {
                    ...
                    if (data.item_total_formatted) {
                        ...
                        if (itemTotalEl) {
                            itemTotalEl.textContent = `${data.item_total_formatted} сум`;
                        }
                    }
